...
class BaseStrategy:
    ...
    def compute(self, df: pd.DataFrame, **kw) -> Dict[str, Any]:
        ...
        Return a dict with keys:
          score: float in [-1..+1] (negative is bearish; 0 neutral; +1 strong long)
          reasons: List[str]
          features: Dict[str,float] (optional)
        Keep it fast: df has columns ts, open, high, low, close, vol.
        """
...
class OscillatorConfluence(BaseStrategy):
    ...
    def compute(self, df: pd.DataFrame, **kw):
        ...
        # shape score with smooth momentum
        s = 0.40*raw + 0.25*_sigmoid((rsi.iloc[-1]-50)/5) + 0.35*_sigmoid(hist.iloc[-1]*6)
        s = _clip(2*s-1, -1, 1)  # map 0..1 to -1..1-ish
        reasons = [f"Stoch K∧D, RSI={rsi.iloc[-1]:.1f}, MACDΔ={hist.iloc[-1]:.3f}"]
        feats = {"rsi": float(rsi.iloc[-1]), "macd_hist": float(hist.iloc[-1]), "k": float(k.iloc[-1]), "d": float(q.iloc[-1])}
...
class TrendSqueeze(BaseStrategy):
    ...
    def compute(self, df: pd.DataFrame, **kw):
        ...
        feats = {"ema_spread": float(ema_f.iloc[-1]-ema_s.iloc[-1]), "bb_bw": float(bw.iloc[-1]) if pd.notna(bw.iloc[-1]) else 0.0}
        return {"score": s, "reasons": reasons, "features": feats}
...
class DonchianBreakout(BaseStrategy):
    ...
    def compute(self, df: pd.DataFrame, **kw):
        ...
        # normalize distance to channel mid
        mid = (hh + ll)/2
        z = (c - mid) / ( (hh-ll)/2 + 1e-12 )
        s = 0.6*raw + 0.4*_sigmoid(float(z.iloc[-1]))
        s = _clip(2*s-1, -1, 1)
        return {"score": s, "reasons": [f"Donchian {self.lookback} breakout"], "features": {"z": float(z.iloc[-1])}}

class VWAPBounce(BaseStrategy):
    name = "vwap_revert"
    def compute(self, df: pd.DataFrame, **kw):
        # intraday approximate VWAP (rolling)
        p = df["close"]; v = df["vol"]
        pv = (p*v).rolling(120).sum()
        vv = v.rolling(120).sum()
        vwap = pv/(vv+1e-12)
        dist = (vwap - p) / (p + 1e-12)
        # buy when price below vwap but rising toward it
        rising = p.diff().iloc[-1] > 0 and p.iloc[-1] < vwap.iloc[-1]
        s = 0.5*_sigmoid(500*float(dist.iloc[-1])) + (0.5 if rising else 0.0)
        s = _clip(2*s-1, -1, 1)
        return {"score": s, "reasons": [f"VWAP revert {float(dist.iloc[-1])*100:.2f}%"], "features": {"vwap_dist": float(dist.iloc[-1])}}

# ----------------- Alpha Router (ensemble + bandit) -----------------
class AlphaRouter:
    """
    Fuses multiple strategies, calibrates to probability, adapts by (symbol,timeframe,regime).
    Exploration via epsilon-greedy; reliability updated externally (via online_learner or ledger hook).
    """
    def __init__(self):
    ...
    def pick(self, df: pd.DataFrame, symbol: str, timeframe: str,
        ...
        for s in self.strats:
            try:
                ...
                # robust to rare calc issues
                votes[s.name] = 0.0

        # map strategy raw scores [-1..1] -> probability via sigmoid
        ...
        # exploration (occasionally boost a strong minority vote)
        ...
        size_mult = _clip(0.5 + 1.5*(p-0.5)/0.5, 0.5, 2.0)  # 0.5 @0.5; up to 2.0 near 1.0
        sleep = 5 if p > 0.7 else 8 if p > 0.6 else 12

        reasons = list(base_reasons)[:3]
        # Append top 2 internal notes
        for n in notes[:2]:
            reasons.append("• " + n)
    ...
    # Optional: external update from PnL ledger to strengthen/penalize
