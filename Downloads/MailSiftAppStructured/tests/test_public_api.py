...
def test_extract_text_from_file_safe():
...
def test_verify_trc20_handles_nonjson(monkeypatch):
    class DummyResp:
        status_code = 200
        text = "not json"
    ...
    monkeypatch.setattr(payments, "requests", types.SimpleNamespace(get=lambda url, timeout=10: DummyResp()))
